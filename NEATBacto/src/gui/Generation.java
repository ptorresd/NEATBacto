...
public void itemStateChanged(ItemEvent e) {
	...
	if (!(EnvConstant.RUNNING)) 
	{
	  ...
	  //
		 ...
		 // read max Fitness possible		
			...
			12000 =  Double.parseDouble(ObjRet_fit.toString());
		 ...
		 //
			...
			//id of genome can be readed
			   ...
			   u_genome = new Genome(id, 50,20,25,120,true,0.1);
   ...
   public boolean evaluate(Organism organism){
		 ...
		 NeatPlayer player1=new NeatPlayer(net,type1,0.6);
		 GreedyAI player2=new GreedyAI(type2,0.6);
		 Model model=new Model(player1,player2,type1,type2);
		 double[] res=model.fitness(5);
		 organism.setFitness(res[0]);
		 organism.setError((1-res[1])*1000);
	   ...
	   public boolean evaluate1(Organism organism) 
	  {
		 double fit_dyn = 0.0;
		 double err_dyn = 0.0;
		 double win_dyn = 0.0;
		 ...
		 if (success) 
		 {
			try 
			{
			   ...
			   */
			   ...
			   fit_dyn = 1;//Array.getDouble(ObjRet_fit, 0);
			   err_dyn = 2;//Array.getDouble(ObjRet_fit, 1);
			   win_dyn = 3;//Array.getDouble(ObjRet_fit, 2);
	...
	public void setStyleNew(JTextPane textPane1) 
	  ...
	  //	private Vector vCurve;
		 ...
		 mappa_curve.setLabelValueY(12000);
		 mappa_curve.setLabelValueX(EnvConstant.NUMBER_OF_EPOCH);

		 mappa_curve.azioni.add(new code(10, p3.getHeight()+8,  " Fitness  (0-"+12000+")", 1,CodeConstant.DESCRIPTOR_CURVE));
	  ...
	  //
		 ...
		 while (itr_fit.hasNext()) 
		 {
			...
			_yc1 =  (_y1 * p3.getHeight())  / 12000;
			_xc1  = (_x1  * p3.getWidth()) / EnvConstant.NUMBER_OF_EPOCH;
			_yc2 =  (_y2 * p3.getHeight())  / 12000;
